...
def parseOATHcsv(csv):
    ...
    log.debug("[parseOATHcsv] the file contains %i tokens." % len(csv_array))
    for line in csv_array:
        ...
        if len(l) >= 1:
            ...
            log.error("[parseOATHcsv] the line %s did not contain a serial number" % line)
            continue
        ...
        log.debug("[parseOATHcsv] read the line |%s|%s|%s|%i %s|%i|" % (serial, key, ttype, otplen, ocrasuite, seconds))
