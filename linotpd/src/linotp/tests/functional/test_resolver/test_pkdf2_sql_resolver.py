# -*- coding: utf-8 -*-
#
#    LinOTP - the open source solution for two factor authentication
#    Copyright (C) 2010 - 2019 KeyIdentity GmbH
#
#    This file is part of LinOTP server.
#
#    This program is free software: you can redistribute it and/or
#    modify it under the terms of the GNU Affero General Public
#    License, version 3, as published by the Free Software Foundation.
#
#    This program is distributed in the hope that it will be useful,
#    but WITHOUT ANY WARRANTY; without even the implied warranty of
#    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
#    GNU Affero General Public License for more details.
#
#    You should have received a copy of the
#               GNU Affero General Public License
#    along with this program.  If not, see <http://www.gnu.org/licenses/>.
#
#
#    E-mail: linotp@keyidentity.com
#    Contact: www.linotp.org
#    Support: www.keyidentity.com
#

"""
sql resolver tests
"""

import logging
import json

from passlib.hash import atlassian_pbkdf2_sha1

from .sql_test_controller import SQLTestController

log = logging.getLogger(__name__)


class SQLResolverPBKDF2Test(SQLTestController):

    def setUp(self):
        """ create an sql user table some users and the sql resolver """

        SQLTestController.setUp(self)
        self.setUpSQL()

        return

    def tearDown(self):
        """ drop the users and the user table """

        self.dropUsers()
        self.delete_all_token()

        return SQLTestController.tearDown(self)

    def test_sqlresolver_with_pbkdf2(self):
        """
        test that we can use pbkdf2 passwords with an sql resolver
        """

        serial = 'PW_PBKDF2'
        realm = 'sqlRealm'

        # ------------------------------------------------------------------ --

        # create the User schema with id field as integer

        self.createUserTable()

        # ------------------------------------------------------------------ --

        # create user bach and brahms with atlasian pkdf2 password

        users = {}

        bach_password = 'bach123'
        users['bach'] = {
            'login': 'bach',
            'uid': '21.3.1685',
            'telephonenumber': '',
            'mobile': 'pw: bach123',
            'surname': 'Bach',
            'givenname': 'Johann Sebastian',
            'password': ('{PKCS5S2}ZYIjvFLd99ldgx5b7sqOlDCKNt31UBX9HQKxTZwU'
                         '50WfuZlWTNG5qBsCsFUMWwxC'),
            'mail': 'j.s@bach.de'
        }

        self.addUser(**users['bach'])

        brahms_password = 'brahms123'
        users['brahms'] = {
            'login': 'brahms',
            'uid': '7.5.1833',
            'telephonenumber': '',
            'mobile': 'pw: brahms123',
            'surname': 'Brahms',
            'givenname': 'Johannes',
            'password': atlassian_pbkdf2_sha1.hash(brahms_password),
            'mail': 'johannes@brahms.de'
        }

        self.addUser(**users['brahms'])

        # ------------------------------------------------------------------ --

        # define resolver and realm

        self.addSqlResolver('my_sql_users')
        self.addSqlRealm(realm, 'my_sql_users', defaultRealm=True)

        # ------------------------------------------------------------------ --

        # create token for user 'hey1'

        params = {
            'type': 'pw',
            'otpkey': 'mein_geheimnis',
            'user': 'bach',
            'realm': realm,
            'serial': serial,
            'pin': 'mypin!'
        }
        response = self.make_admin_request('init', params=params)

        self.assertTrue('false' not in response.body, response)

        # ------------------------------------------------------------------ --

        # run a wrong login, so that the token failcount increments

        params = {
            'user': 'bach',
            'pass': 'mypin!kein_geheimnis'
            }

        response = self.make_validate_request('check', params=params)
        self.assertTrue('"value": false' in response)

        # ------------------------------------------------------------------ --

        # verify that the token count is incremented to 1

        params = {
            'serial': serial
            }

        response = self.make_admin_request('show', params=params)
        jresp = json.loads(response.body)
        token_info = jresp.get(
            'result', {}).get(
                'value', {}).get(
                    'data',[{}])[0]
        self.assertTrue(token_info.get( "LinOtp.FailCount", -1) == 1)

        # ------------------------------------------------------------------ --

        # now login to the selfservice and run the token reset

        # run a wrong login, so that the token failcount increments

        params = {
            'user': 'bach',
            'pass': 'mypin!mein_geheimnis'
            }

        response = self.make_validate_request('check', params=params)
        self.assertTrue('"value": true' in response)


        # ------------------------------------------------------------------ --

        # create the policy to check for password instead of pin

        params = {
            'name': 'otppin_poilcy',
            'action': 'otppin=password',
            'scope': 'authentication',
            'user': '*',
            'realm': '*',
            }

        response = self.make_system_request('setPolicy', params=params)
        self.assertTrue('false' not in response.body)

        # ------------------------------------------------------------------ --

        # verify that correct password works

        params = {
            'user': 'bach',
            'pass': 'bach123mein_geheimnis'
            }

        response = self.make_validate_request('check', params=params)
        self.assertTrue('"value": true' in response)

        # verify that wrong password works

        params = {
            'user': 'bach',
            'pass': 'bach1231mein_geheimnis'
            }

        response = self.make_validate_request('check', params=params)
        self.assertTrue('"value": false' in response)

        return

# eof
