...
class TestChallengeResponseController(TestingChallengeResponseController):
    ...
    def test_03_hmac_regression(self):
        ...
        otpkey = "AD8EABE235FC57C815B26CEF3709075580B44738"
        # normal test
        serial = self.createToken(pin="shortpin", otpkey=otpkey, user="passthru_user1")
    ...
    def test_02_spass_regression(self):
        ...
        # normal test
        serial = self.createToken(pin="shortpin", typ="spass", user="passthru_user1")
        ...
        serial = self.createToken(pin="otppin", typ="spass", user="passthru_user1")
    ...
    def test_10_multiple_tokens(self):
        ...
        for _i in range(1, 3):
            ...
            response2 = self.make_validate_request(action="check", params=params)
    ...
    def test_13_dynamic_mobile_number(self):
        ...
        # normal test
        serial = self.createToken(
            pin="shortpin", typ="sms", phone="12345", user="passthru_user1"
        )
    ...
    def test_14_sms_with_check_s(self):
        ...
        serial = self.createToken(
            pin="shortpin", typ="sms", phone="12345", otpkey=otpkey
        )

        params = {"serial": serial, "realms": "mydefrealm"}

        response = self.make_admin_request(action="tokenrealm", params=params)
        ...
        response = self.make_system_request(
            action="getPolicy",
            params={"name": "trigger_sms", "session": self.session},
        )
    ...
    def test_50_hmac_auth(self):
        ...
        serial = self.createToken(
            pin="shortpin", typ="hmac", otpkey=otpkey, user="passthru_user1"
        )
        self.make_admin_request(
            action="set", params={"pin": "shortpin", "serial": serial}
        )
    ...
    def test_60_hmac_two_tokens(self):
        ...
        * passthru_user1@myDefRealm - using token2
        ...
        counter2 = self.do_auth(
            "", counter2 + 1, otpkey=otpkey2, user="passthru_user1@myDefRealm"
        )

        self.delete_policy("otpPin")

        counter1 = self.do_auth(
            "shortpin",
            counter1 + 1,
            otpkey=otpkey1,
            user="passthru_user1@myMixRealm",
        ...
        counter1 = self.do_auth(
            "", counter1 + 1, otpkey=otpkey1, user="passthru_user1@myMixRealm"
        )

        self.delete_policy("otpPin")

        counter2 = self.do_auth(
            "shortpin", counter2 + 1, otpkey=otpkey2, user="passthru_user1"
        )
        ...
        counter2 = self.do_auth(
            "geheim1", counter2 + 1, otpkey=otpkey2, user="passthru_user1"
        )
        ...
        counter2 = self.do_auth("", counter2 + 1, otpkey=otpkey2, user="passthru_user1")
    ...
    def test_62_email_active_inactive_tokens(self):
        ...
        counter = 0
        # email token should do challenge response even without policy
        counter = self.do_email_auth(
            "shortpin", counter, typ=typ, user="passthru_user1"
        )
        ...
        counter = self.do_email_auth("geheim1", counter, typ=typ, user="passthru_user1")
        ...
        counter = self.do_email_auth("", counter, typ=typ, user="passthru_user1")
    ...
    def test_63_sms_config_error(self):
        ...
        sms_conf = {
            "mailserver": "somemailserverthatdoesntexist.com",
    ...
    def test_72_exception_in_challenge(self):
        ...
        params = {"user": "passthru_user1", "pass": "shortpin"}
        response = self.make_validate_request(action="simplecheck", params=params)
    ...
    def test_challenge_response_auto_resync(self):
        ...
        params = {"user": "passthru_user1", "pass": pin + otp}
        response = self.make_validate_request(action="check", params=params)
        assert '"value": true' in response, response
